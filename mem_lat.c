...
int main(int argc, char *argv[])
{
    ...
    //unsigned long count = 5000000;   // how many  p = *p 
    ...
    // Seed random and shuffle with Fisher-Yates
    srand(time(NULL));
    for (i = size - 1; i > 0; i--) {
        j = rand() % (i + 1);
    ...
    *(char **)&mem[indices[size - 1] * stride] = (char *)&mem[indices[0] * stride];
    ...
    printf("Buffer size: %ld KB, stride %d, time %ld.%06ld s, latency %.2f ns, %ld tsc\n",
           memsize / 1024, stride, sec, usec, (sec * 1000000 + usec) * 1000.0 / (tmp * 100), (t1-t0) / (tmp * 100));
    munmap(mem, memsize);
    free(indices);
}